...
%% Train SVM
[alpha, w0] = trainSVM(X_train, t_train, @linearkernel);
w = X_train' * (alpha.*t_train);

%% Evaluate SVM
y = discriminant(alpha, w0, X_train, t_train, X_test, @linearkernel);
...
hold on 
plotHyperplane2D(X_train, t_train, alpha, w0, @linearkernel);
hold off

%% Plot Data 3D
plotData3D(X_train, t_train);
hold on
plotHyperplane3D(X_train, t_train, alpha, w0, @linearkernel);
hold off